/*
  ...
  Note: Adding an SSID will Automatically ENABLE WiFi attempt to Log-In.
    ...
    * Upload a BIN file. Compiled from PlatformIO, in Folder:  [Code Root Folder] > pio > build > esp32-c3-devkitm-1 > firmware.bin
*/
...
void WebServerPageContent(void)
{
  ...
  page_time = "<p>" + AppAcronym + " Time: <strong>" + RTC_Time + " </strong><small>(Auto Refresh)</small></p>";
  ...
  page_detail = "";
  page_detail = page_detail + "<tr><td align='left'>Temperature </td><td align='right'><strong>" + String(BME280_Temperature) + "</td><td align='left'> &#xb0;C</strong></td></tr>";
  page_detail = page_detail + "<tr><td align='left'>Air Quality CO2 </td><td align='right'><strong>" + String(SCD4x_CO2) + "</td><td align='left'> ppm</strong></td></tr>";
  page_detail = page_detail + "<tr><td align='left' width='40%'>Barometer</td><td align='right'width='35%'><strong>" + String(BME280_Pressure) + "</td><td align='left' width='25%'> hPa/mb </strong></td></tr>";

  page_detail_footer = "</table>";

  page_temperature = "<p><h2>Probe Temperature 1:  <strong>" + String(DS_TemperatureC[0]) + " &#xb0;C<BR>Probe Temperature 2:  <strong>" + String(DS_TemperatureC[1]) + " &#xb0;C</strong></h2></p>";
